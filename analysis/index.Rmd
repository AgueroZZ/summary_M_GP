...
$$\overset{..}{f}(t_i) =  \mathbf{\Phi}(t_{i-1},t_i)\overset{..}{f}(t_{i-1}) + \int_{t_{i-1}}^{t_i}\mathbf{\Phi}(\tau,t_i)\mathbf{L}w(\tau)d\tau.$$
...
$$\mathbf{\Phi}(\tau,t) = \begin{bmatrix} -[\frac{\partial}{\partial t}G_{\tau}^*(t) + \alpha(t)G_{\tau}^*(t)] & G_{t}^*(\tau) \\ -[\frac{\partial^2}{\partial t\partial\tau}G_{\tau}^*(t) + \alpha(t)\frac{\partial}{\partial \tau}G_{\tau}^*(t)] & \frac{\partial}{\partial \tau}G_{t}^*(\tau) \end{bmatrix}.$$
Here $G_{\tau}^*(t)$ is the Green's function of the operator $L_\alpha^*$ (i.e. the adjoint of $L_\alpha$).
Specifically, the Green function is $G_{\tau}^*(t) = \frac{1}{h^*(\tau)}[g^*(\tau) - g^*(t)]$, where $g^* = D^{-1} h^*$ and $h^* = \exp(D^{-1}\alpha^*)$, and $\alpha^* = \frac{1}{a(x+c)}$.
