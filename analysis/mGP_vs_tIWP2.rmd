...
Specifically, let's assume we have a function $f$ defines for $t \in [0,11]$, where $f(9)$ is known to be 0 and $f'(9)$ is known to be 0.002.
...
In other words, if $f(x)$ is a M-GP, $f(-x)$ is not a M-GP generated by the same $\alpha(x)$.
For example, when $f(x) = \sqrt{x}$ and $\tilde{x} = x - c$, we can define $f_1(\tilde{x}) = \sqrt{\tilde{x} +c} = \sqrt{(x-c)+ c} = \sqrt{x} = f(x)$.
But if we define $\underset{\sim}{x} = c-x$, then $f_2(\underset{\sim}{x}) = \sqrt{c-\underset{\sim}{x}} = \sqrt{c-c+x} = f(x)$. 
The form of $\sqrt{c-\underset{\sim}{x}}$ can not be written as the base model of any M-GP!
...
result1_part2[,1] <- (x_trans_part2_relative + m(c))^2
```
