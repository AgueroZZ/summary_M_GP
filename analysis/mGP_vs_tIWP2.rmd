...
In other words, if $g(x)$ is a M-GP, $g(-x)$ is not a M-GP.
For example, when $g(x) = \sqrt{x+c}$ and $\tilde{x} = x - c_1$, we can define $g_1(\tilde{x}) = \sqrt{\tilde{x} + c_1 +c} = \sqrt{(x-c_1)+c_1 + c} = \sqrt{x+c} = g(x)$.
But if we define $\underset{\sim}{x} = c_1-x$, then $g_2(\underset{\sim}{x}) = \sqrt{c_1+c-\underset{\sim}{x}} = \sqrt{x+c} = g(x)$. 
The form of $\sqrt{c_1+c-\underset{\sim}{x}}$ can not be written as the base model of any M-GP!

We need to find which model $g(-x)$ should correspond to...
...
This becomes an easier problem, as the reverse process of IWP2 is still an IWP2 with an opposite derivative (i.e. $g(x) \sim \text{IWP}$ implies $g(-x) \sim \text{IWP}$).

This implies that we just need to merge the forward IWP and the backward IWP together, while being careful with the initial condition ($g'(c) = \tilde{g}'(m(c))m'(c)$, so $\tilde{g}'(m(c)) = g'(c)/m'(c)$).
