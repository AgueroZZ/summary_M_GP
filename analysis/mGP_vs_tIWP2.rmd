...
B = 100
c <- 1.1
samps0 <- mGP_sim(mesh_size = 0.01, max_t = 5, alpha = (2), c = c)
result0 <- samps0[,1:2]
for (i in 1:B) {
  samps0 <- mGP_sim(mesh_size = 0.01, max_t = 5, alpha = (2), c = c)
  result0 <- cbind(result0, samps0[,2])
}
matplot(x = result0[,1], y = result0[,-1], type = 'l', ylab = "Post Samp", xlab = "x", main = "alpha = 2", ylim = c(-10,10))
```
...
# sampling from tIWP2
samps1 <- sim_IWp_Var(t = t_trans, p = 2, sigmaS = 1)
result1 <- cbind(c(t), samps1[,2])
for (i in 1:B) {
  samps1 <- sim_IWp_Var(t = t_trans, p = 2, sigmaS = 1)
  result1 <- cbind(result1, samps1[,2])
}
matplot(x = result1[,1], y = result1[,-1], type = 'l', ylab = "Post Samp", xlab = "x", main = "tIWP2", ylim = c(-10,10))
```
